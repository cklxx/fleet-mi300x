...
Author: Kailun Chen · Status: v0.18 (measured on the MI300X; v0.8 is the submitted proposal, kept verbatim in `docs/task/`) · Target: single MI300X (gfx942), BF16, bs=1, 1024-token context, 32 greedy tokens
...
Implementation note (v0.18, what runs today; since v0.18 each head's absorbed q_c is also published once by the workers that would otherwise idle through the attention phase, rather than every KV chunk streaming all 131 KB of W_UK to rebuild it, which is worth 1.5%; `src/host/taskgraph.py --report`): the graph above is the v0.9 one. The measured kernel runs a **three-global-event** variant: q rows are head-aligned per XCD and the 576 kv_a rows are split over the 8 XCDs into one buffer behind a global event (`--kva-replicated` restores the XCD-local variant with 8 copies, measured 0.6% slower); the merge + W_UV of a head is spread over its kv_chunks tasks (16); **o_proj is K-split per XCD** (each XCD multiplies its own heads' 256 columns of W_o against its own o slice, so merge → o_proj is XCD-local) and its 8 fp32 partials are folded by every router worker in its prologue (`--oproj-row-split` builds the alternative -- W_o's output rows split over the XCDs against the full o[2048], no partials, but a global merge boundary and K 2048; prepared and unmeasured, STATUS.md); the router is a Chiplet-task per XCD (experts wait XCD-locally and pick top-k themselves); there is no REDUCE — the next layer's q/kv_a workers (and lm_head's) fold the 8 expert partials in their prologue, and worker (0,0) publishes the folded vector to the other of two `x` buffers. Global events per MoE layer: q/kv_a → attention, o_proj → router, down → next layer; 85 per token, 1,157 events in all. Per MoE layer: 552 logical tasks, 1,992 descriptors. The expert down reuses the routing its worker's gate_up left in LDS (the validator checks the queue order). The costs of that choice (kv_a replication 0.51 GB/token, the partial fold read by every worker) and the variants measured against it (fold by the last arriver, tiled gate_up→down, prefetch) are in STATUS.md.
...
Memory model (v0.13, what the code actually does): MI300X L2 is per XCD and not coherent across XCDs for ordinary device memory, so every handshake follows the LLVM AMDGPU memory model for gfx942 rather than cache folklore. Producers drain their stores and bump an XCD-local arrival counter; the arrival that completes the XCD's share of a global event releases once (`buffer_wbl2 sc1`, the Fleet last-arriver flush, bench (d')) and adds the share to the agent-scope global counter; pollers use agent-scope atomic loads; consumers of a global event acquire with `buffer_inv sc1`, consumers of an XCD-local event invalidate only their L1 (`buffer_inv sc0`: the payload is in the producer's own L2). Waits carry a spin limit: a wait that never completes ends the launch with the event id instead of hanging the GPU. `--coherent-acts` (experimental) accesses every cross-XCD activation through agent-scope relaxed atomics (`global_load/store … sc1`, the model's L2 bypass, bench (d''')) and drops the fences on global events except at the token boundary (the KV cache row, whose writer flushes explicitly) and the dense layer's h; `hipDeviceMallocUncached` memory was measured *not* to give this on the VM (bench (d''): 75% stale words).
...
| Tuned target | 2.5–3.5 ms/token | **3.60 ms (278 tok/s), 32/32 tokens, 27/27 layers** — ~1.4 TB/s at ~5.2 GB/token; the loss is serial phases and their prologues, not bytes or events (STATUS.md). vLLM on the same box: 4.52 ms |
| Protocol only, per token | ~0.3–0.7 ms | 2.84 ms with the 1,346 events of the v0.16 graph (1,589 today) (2.20 ms without fences): the smoke run has no bodies, so every wait is back-to-back |
