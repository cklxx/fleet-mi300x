...
// Static model shape, read from build/weights.json by the host.
struct ModelDims {
    int hidden, heads, q_head_dim, qk_nope, qk_rope, v_head;
    int kv_lora, moe_inter, dense_inter, n_routed, top_k, vocab;
    int layers, first_k_dense, seq_len;
    float rms_eps, softmax_scale;
};
...
struct Weights {
    const __hip_bfloat16* const* layer_q_proj;
    const __hip_bfloat16* const* layer_kv_a;
...
struct Activations {
    ...
    float* __restrict__ x_norm;        // [hidden] fp32
    float* __restrict__ q;             // [heads * q_head_dim]
    float* __restrict__ kv_a;          // [kv_lora + qk_rope] raw, pre-norm
    float* __restrict__ q_c;           // [heads][kv_lora] absorbed queries
    float* __restrict__ attn_partial;  // [heads][chunks][2 + kv_lora] m, l, acc
    float* __restrict__ o;             // [heads * v_head]
    float* __restrict__ expert_h;      // [XCDs][moe_inter * 2]
